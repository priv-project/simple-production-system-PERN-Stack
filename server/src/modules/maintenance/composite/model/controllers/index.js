...
export const createModel = async (req, res) => {
    ...
    try {
        const oldModel = await pool.query('SELECT * FROM model WHERE model_code = $1', [model_code]);
        if (oldModel.rowCount > 0) return res.status(500).json({ message: 'Model already exist!' });
        const lastInsertId = await pool.query(`INSERT INTO model (model_code, model_description) VALUES ($1, $2) RETURNING model_id`, [
            model_code,
            model_description
        ]);
...
export const updateModel = async (req, res) => {
    ...
    try {
        ...
        if (!model) return res.status(404).json({ message: `No model with id: ${id}` });
...
export const deleteModel = async (req, res) => {
    const { id } = req.params;
    try {
        const result = await pool.query(`DELETE FROM model WHERE model_id = $1`, [id]);
...
const getModelById = async (id) => {
    const result = await pool.query(`SELECT * FROM model WHERE model_id = $1`, [id]);
