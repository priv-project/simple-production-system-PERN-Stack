...
export const signin = async (req, res) => {
    ...
    try {
        const oldUser = await pool.query('SELECT * FROM users WHERE user_username = $1', [user_username]);
        if (oldUser.rowCount === 0) return res.status(404).json({ message: "User doesn't exist" });
        const isPasswordCorrect = await bcrypt.compare(user_password, oldUser.rows[0].user_password);
...
export const signup = async (req, res) => {
    ...
    try {
        const oldUser = await pool.query(`SELECT * FROM users WHERE user_username = $1 LIMIT 1`, [user_username]);
        if (oldUser.rowCount > 0) return res.status(400).json({ message: 'User already exists' });
